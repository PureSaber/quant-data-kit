...
class CaptureStreamRunner:
    def __init__(
        ...
        *,
        ...
        self.state = CaptureStateMachine(
            ...
            collector_commit=config.collector_commit,
            clock=clock,
            audit_sink=self.segment_writer.append,
    ...
    async def run(self, *, maximum_websocket_messages: int | None) -> CaptureStreamReport:
        ...
        while self.state.state is not CaptureState.PAUSED:
            ...
            try:
                ...
                if maximum_websocket_messages is not None:
                    self.state.transition(CaptureState.PAUSED, "bounded_probe_complete")
                ...
                self._pause("capture_cancelled")
                raise
            except Exception as exc:  # noqa: BLE001 - all feed failures require audit/retry
                failed_attempts += 1
                ...
                if self.state.state is CaptureState.PAUSED:
                    self._alert(
                        f"CAPTURE_PAUSED:{self.stream.stream_id}:post_pause_failure:"
                ...
                try:
                    self._drain_segments()
                except Exception as archive_exc:  # noqa: BLE001 - archive failures always pause
                    self._errors.append(f"{type(archive_exc).__name__}: {archive_exc}")
                    self._pause("archive_failure", archive_exc)
                    break
                self._finalize_epoch(abort_reason=str(exc))
                if failed_attempts >= self.config.retry.max_attempts:
                    self._pause("retry_budget_exhausted", exc)
                    break
        ...
        if self.segment_writer.pending_messages:
            self.segment_writer.flush()
            try:
                self._drain_segments()
            except Exception as exc:  # noqa: BLE001 - terminal archive failure remains visible
                self._errors.append(f"{type(exc).__name__}: {exc}")
                self._alert(
                    f"TERMINAL_ARCHIVE_FAILURE:{self.stream.stream_id}:{type(exc).__name__}:{exc}"
                )
        self._finalize_epoch()
        return self._report()
    ...
    async def _receive_frame(self, connection: WebSocketConnection) -> RawFrame:
        payload = await connection.receive(timeout_seconds=self.receive_timeout_seconds)
    ...
    def _https_frame(
        ...
        *,
        ...
        try:
            ...
            raise ResyncRequired("Binance HTTPS snapshot is malformed") from exc
        return RawFrame(
    ...
    def _apply_outcome(self, outcome: NormalizationOutcome) -> None:
        ...
        for observation in outcome.observations:
            self._observations[observation.event] = self._observations.get(observation.event, 0) + 1
            self.state.audit(observation.event, observation.reason, observation.details)
    ...
    def _drain_segments(self) -> None:
        for segment in self.segment_writer.drain_completed():
    ...
    def _finalize_epoch(self, abort_reason: str | None = None) -> None:
        ...
        if abort_reason and not journal.records:
            journal.abort_visible(abort_reason)
            return
...
class CryptoL2CaptureCoordinator:
    ...
    async def run(self, *, maximum_websocket_messages: int | None) -> CaptureRunReport:
        ...
        status = (
            ...
            if maximum_websocket_messages is not None
            else "STOPPED_WITHOUT_CERTIFICATION"
        )
        return self._write_report(
            run_id,
            started,
    ...
    def _write_report(
        ...
        *,
        ...
        identity = {
            "schema_version": "puresaber.crypto-l2-capture-run@1.0.0",
            "run_id": run_id,
